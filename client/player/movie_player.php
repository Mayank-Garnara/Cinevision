...
$user_id = $_SESSION['user']['id'];  // Replace with actual user ID
$movie_id = $movie['id'];  // Replace with actual movie ID

// Check if the record exists
$stmt = $pdo->prepare("SELECT COUNT(*) FROM watch_history WHERE user_id = :user_id AND movie_id = :movie_id");
$stmt->execute([':user_id' => $user_id, ':movie_id' => $movie_id]);
$exists = $stmt->fetchColumn();

if (!$exists) {
    // Insert only if it doesn't exist
    $insert = $pdo->prepare("INSERT INTO watch_history (user_id, movie_id) VALUES (:user_id, :movie_id)");
...
<body>
    ...
    <script>
        ...
        player.ready(function () {
            player.src({
                src: '../../uploads/movies/<?=$movie['id']?>/video/movie/<?=$movie['id']?>.mpd',
